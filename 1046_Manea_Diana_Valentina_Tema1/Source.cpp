...
public:
	...
	Apartament(const Apartament&) = delete;
	...
	static float mediaPreturilor(Apartament preturi[], int nrApartamente) {
		float totalPret = 0;
		float media;
		for (int i = 0; i < nrApartamente; i++) {
			totalPret += preturi[i].pret;
		}
		media = totalPret / float(nrApartamente);
		return media;
	}
...
int main() {
	...
	const int nrApartamente = 2;

	Apartament va[nrApartamente];
	va[0] = a1;
	va[1] = a2;
	cout << "Media preturilor apartamentelor a1 si a2 este: " << Apartament::mediaPreturilor(va, nrApartamente) << endl;
